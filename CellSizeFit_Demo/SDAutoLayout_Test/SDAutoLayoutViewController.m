...
@interface SDAutoLayoutViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation SDAutoLayoutViewController
...
- (void)testPerformance
{
    ...
    [self.timer timerExecute:^{
        ...
        // 随机得到 25% 的cell，去随机改变cell数据，测试SDAutoLayout性能
...
{
    SDAutoLayoutTableViewCell *cell = [SDAutoLayoutTableViewCell cellWithTableView:tableView];
...
{
    Class currentClass = [SDAutoLayoutTableViewCell class];

    TimeLineModel *lineModel = self.timeLineArr[indexPath.row];
    
    CGFloat height = [self.tableView cellHeightForIndexPath:indexPath model:lineModel keyPath:@"lineModel" cellClass:currentClass contentViewWidth:[UIScreen mainScreen].bounds.size.width];
